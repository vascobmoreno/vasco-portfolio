...
export default function QuickMath({ onSolve }: { onSolve: () => void }) {
  ...
  return (
    <div className="flex flex-col items-center gap-8 w-full">
      <div className="font-mono text-5xl text-white tracking-wider">{q}</div>
      ...
      <form onSubmit={submit} className="flex flex-col items-center gap-4">
        <input
          ...
          style={{ borderColor: shake ? '#ff4444' : '#00ffcc55', transform: shake ? 'translateX(-3px)' : 'none', transition: 'all 0.15s' }}
          placeholder="?"
        />
        <button type="submit"
